  ...
  const queryData = {
    query:
        `PREFIX news:<http://example.co.id/ns/newsdata>
            ...
            FILTER contains(lcase(str(?category)), lcase(str("${value.title}")))
    }`
  };
...
const categoryHandler = event => { //handleChangeDate
  setValue({
    ...value,
...
return (
  ...
  <div class="result">
    <div class="then-right">
      ...
      <div class="column-left">
        <div class="dropdown">
          <select setValue={value.category} className="dropbtn" id="category" onChange={categoryHandler}>
